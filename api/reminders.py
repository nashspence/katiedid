...
from temporal_schedule_input import parse_schedule_json
...
def _sch(schedule_id: str, component_json: dict | str | None, wf: str, args: list) -> Schedule:
    payload = component_json if isinstance(component_json, dict) else json.loads(component_json or "{}")
    spec, policy, state = parse_schedule_json(json.dumps(payload))

    return Schedule(
        action=ScheduleActionStartWorkflow(
            wf,
            id=schedule_id,          # workflow execution id (you use schedule id as workflow id)
...
async def _del(c: Client, sid: str) -> None:
    try:
        await c.get_schedule_handle(sid).delete()
    except Exception:
        pass
...
async def _upsert(c: Client, sid: str, sch: Schedule) -> None:
    ...
    try:
        await c.create_schedule(sid, sch)
    except ScheduleAlreadyRunningError:
        # Update callback can be sync; Temporal will handle conflicts internally.
        def updater(_inp):
            return ScheduleUpdate(schedule=sch)
        await h.update(updater)


async def _next_time(c: Client, sid: str):
    try:
...
async def _db_put(kind, payload):
    if not _dbp or not _dbp.is_alive():
        raise RuntimeError("db process died")
    ...
    m = await fut
    if not m.get("ok"):
        raise RuntimeError(m.get("err") or "db error")
...
async def _proc(pg, c: Client, row):
    ...
    try:
        ...
        if sid.startswith("reminder-"):
            ...
            sch_obj = _sch(
                ...
                sch,
                WF_INBOX,
                [{"kind": "fire_reminder", "payload": {"reminder_id": rid, "sid": sid}}],
            )
        else:
